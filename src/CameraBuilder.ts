...
export default class CameraBuilder {
  ...
  public get isIOSPriorTo17(): boolean {
    return (
      navigator.userAgent.includes("iPhone OS") &&
      !navigator.userAgent.includes("iPhone OS 17")
    );
  }
  ...
  public static initWithUserCamera(): CameraBuilder {
    const instance = new CameraBuilder(
      MediaConstraints.userCameraConstraints()
    );
    return instance;
  }
  /**
   ...
   */
  public withCameraStreamListener(
    cameraStreamListener: (MediaStream) => void
  ): CameraBuilder {
  ...
  public pick4KResolution(): CameraBuilder {
    if (this.mediaConstraints.video.aspectRatio > 1 || !this.isIOSPriorTo17) {
